...
public class TreeItemsEventHandler extends AbstractHandler{

	@Override
	public Object execute(ExecutionEvent event) throws ExecutionException {

		ISelectionService selectionService = Workbench.getInstance().getActiveWorkbenchWindow().getSelectionService();
		ISelection selection  = selectionService.getSelection("pattern01.plugin.components.navigator.PatternNavigator");

		
		String clas = selection.getClass().getName();
//		if (selection instanceof IStructuredSelection){
//			Object element = ((IStructuredSelection)selection).getFirstElement();
//			if (element instanceof IAdaptable){
		
				//Se chequea 
				if (event.getCommand().getId().equalsIgnoreCase("Pattern01.addItemCommand")){
					
				}else if(event.getCommand().getId().equalsIgnoreCase("")){
